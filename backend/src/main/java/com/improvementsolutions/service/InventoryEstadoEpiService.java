...
import com.improvementsolutions.repository.InventoryEstadoEpiRepository;
...
public class InventoryEstadoEpiService {

    private final InventoryEstadoEpiRepository repository;

    private EppCatalogDto toDto(InventoryEstadoEpi entity) {
    ...
    public EppCatalogDto create(EppCatalogDto input) {
        ...
        InventoryEstadoEpi entity = new InventoryEstadoEpi();
        entity.setName(name);
    ...
    private String requireName(EppCatalogDto input) {
        ...
        if (name.length() > 120) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "El nombre no puede superar 120 caracteres");
        }
        return name;
    }
