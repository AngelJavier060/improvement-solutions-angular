...
import { AttendanceService, PermissionRecord } from '../services/attendance.service';
...
export class PermisosComponent implements OnInit {
  ...
  readonly permissionTypes = ['ESTUDIOS', 'CALAMIDAD', 'PERSONAL', 'MEDICO', 'OTRO'];
  ...
  loadRecords(): void {
    ...
    this.loading = true;
    this.attendanceService.getPermissions(this.businessId, this.filterYear, this.filterMonth).subscribe({
  ...
  selectEmployee(emp: EmployeeResponse): void {
    ...
    this.showForm = true;
    this.form.reset({ permissionDate: '', permissionType: 'PERSONAL', hoursRequested: 8, reason: '', notes: '', status: 'APROBADO' });
    this.error = null;
  }
  ...
  submitForm(): void {
    ...
    this.saving = true;
    this.error = null;
    this.attendanceService.savePermission(this.businessId, this.selectedEmployee.id, this.form.value).subscribe({
      next: () => {
        this.saving = false;
        this.showForm = false;
        this.selectedEmployee = null;
        this.successMsg = 'Permiso registrado correctamente';
