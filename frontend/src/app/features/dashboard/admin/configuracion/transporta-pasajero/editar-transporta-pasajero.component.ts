...
@Component({
  selector: 'app-editar-transporta-pasajero',
  templateUrl: './editar-transporta-pasajero.component.html',
  styleUrls: ['./editar-transporta-pasajero.component.scss']
})
export class EditarTransportaPasajeroComponent implements OnInit {
  form: FormGroup;
  loading = true;
  saving = false;
  error = '';
  ...
  onSubmit(): void {
    ...
    const entity: TransportaPasajero = {
    ...
    this.service.update(this.id, entity).subscribe({
      next: () => this.router.navigate(['/dashboard/admin/configuracion/transporta-pasajero']),
      error: (err) => {
