...
@Component({
  selector: 'app-editar-medio-comunicacion',
  templateUrl: './editar-medio-comunicacion.component.html',
  styleUrls: ['./editar-medio-comunicacion.component.scss']
})
export class EditarMedioComunicacionComponent implements OnInit {
  form: FormGroup;
  loading = true;
  saving = false;
  error = '';
  ...
  onSubmit(): void {
    ...
    const entity: MedioComunicacion = {
    ...
    this.service.update(this.id, entity).subscribe({
      next: () => this.router.navigate(['/dashboard/admin/configuracion/medio-comunicacion']),
      error: (err) => {
