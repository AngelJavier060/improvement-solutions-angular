...
export class NuevaCondicionClimaticaComponent implements OnInit {
  form: FormGroup;
  loading = false;
  ...
  onSubmit(): void {
    ...
    const entity: CondicionClimatica = {
    ...
    this.service.create(entity).subscribe({
      next: () => this.router.navigate(['/dashboard/admin/configuracion/condicion-climatica']),
      error: (err) => {
