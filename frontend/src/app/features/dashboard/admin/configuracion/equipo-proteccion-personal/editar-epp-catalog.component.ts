...
@Component({
  selector: 'app-editar-epp-catalog',
  templateUrl: './editar-epp-catalog.component.html',
  styleUrls: ['./epp-catalog.shared.scss']
})
export class EditarEppCatalogComponent implements OnInit {
  form: FormGroup;
  submitting = false;
  loading = true;
  ...
  ngOnInit(): void {
    ...
    this.editarSubtitulo = this.catalogApi.isNameOnly(this.catalogKey)
      ? (this.catalogKey === 'estado-epi'
        ? 'Actualice nombre y descripción. Ej.: Buen estado, Desgaste normal, Deteriorado…'
        : this.catalogKey === 'tipo-acontecimiento'
        ? 'Actualice nombre y descripción. Ej.: Solicitud de EPP, Incumplimiento, Deterioro…'
        : this.catalogKey === 'tipo-salida'
          ? 'Actualice nombre y descripción. Ej.: Entrega de EPP a trabajador, Préstamo, Baja…'
          : 'Actualice nombre y descripción. Ej.: Compra, Devolución, Transferencia, Ajuste o Donación.')
      : 'Actualice nombre, código y descripción.';
    ...
    this.itemId = id;
    this.catalogApi.getById(this.catalogKey, id).subscribe({
      next: (item: EppCatalogItem) => {
  ...
  onSubmit(): void {
    if (this.form.invalid || !this.catalogKey || !this.itemId) {
    ...
    this.catalogApi.update(this.catalogKey, this.itemId, this.form.value).subscribe({
      ...
      error: (err: any) => {
        this.error = err?.status === 409
          ...
          : (err?.error?.message || err?.message || 'Error al actualizar. Intente nuevamente.');
        this.submitting = false;
      }
    });
  }
